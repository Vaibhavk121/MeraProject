...
const boxClassVariable = "flex h-full items-center justify-center rounded-lg text-xl font-medium drop-shadow-lg bg-rose-500";
...
export default function AnimationLayout() {
    ...
    const [sizeFullMsg, setSizeFullMsg] = useState(false)
  ...
  let timeline = gsap.timeline();
    ...
    const onPushOperation = contextSafe(()=>{
      if(top >= arrayLength){
        setSizeFullMsg(true)
        setTimeout(()=>{
          setSizeFullMsg(false)
        },2000)
        return
      }
        setElementScanner(true)
        setTimeout(() => {
            timeline.fromTo(`.box${top}`,
    ...
    const popButtonHandler = ()=>{
      ...
      setTimeout(()=>{
        const newArray = [...mainArray];
        newArray[top] = {id: top.toString() ,value:-1}
        setMainArray(newArray)
      },100)
    }
    ...
    const onPeekBtnHandler = () =>{
      setTimeout(()=>{
        gsap.fromTo(`.box${top-1}`,
          {scale:1.5,duration:2,ease:"back.inOut",backgroundColor:"red"},
          {scale:1,duration:1,ease:"back.inOut",backgroundColor:"rgb(244 63 94 / var(--tw-bg-opacity))"})
      },1)
    }
    ...
    >
      <div className="flex h-[80vh] w-[95%] rounded-lg border-[2px] border-white">
        ...
        <div className="flex relative items-center justify-center flex-col-reverse p-2 w-full overflow-hidden corn">
          {sizeFullMsg && <p className="text-2xl font-medium absolute bottom-5">Array size is Full</p>}
        {defaultText && (
            ...
            {elementScanner && <div className="scan absolute  p-1 rounded-md left-[30%] -bottom-5 w-[10rem] mb-[2rem] h-[5rem] flex items-center gap-1 text-xl">
              Top<FaArrowRightLong size={30} />
              </div>}
            {mainArray.map((ele, i) => (
                <div key={i} className="">
